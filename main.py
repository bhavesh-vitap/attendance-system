...
'''creds = Credentials.from_service_account_file('serviceAccountKey.json', scopes=['https://www.googleapis.com/auth/cloud-platform'])
...
while True:
    ...
    '''if __name__ == '__main__':
        t1 = threading.Thread(target=capture_it, args=(check_it,))
        t2 = threading.Thread(target=checkForStop)
...
if temp == 0 and check == 0:
    check += 1
    docs = db.collection("Faculty").document("Faculty ID").collection("Class ID").where("time", "<=", time_global).get()
    for doc in docs:
        key = doc.id
        db.collection("Faculty").document("Faculty ID").collection("Class ID").document(key).delete()
    ...
    f1 = open(filename, "w+")
    f1.close()
    #db.collection(u'Faculty').document(u'Faculty ID').delete()
    #db.collection(u'faculty').document(u'Faculty 1').delete()
    #delete_collection(db.collection(u'Faculty'), 1)
